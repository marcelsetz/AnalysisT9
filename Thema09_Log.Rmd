...
```{r}
ML_dataLoc <- "data/Smoker_Epigenetic_dfExperiment.csv"
...
knitr::kable(avgs_df)
write.csv(avgs_df, file = "data/Smoker_Epigenetic_df_Summary.csv")

```
![The generated tree of the J48 algorithm. Apparently these CpG islands say the most about whether a person smokes or not.](data/J48Tree.png)
...
It's quite obvious that every algorithm except for NaiveBayes are very close to eachother, the only significant lower scores according to weka are NaiveBayes and OneR, the best (not really significant) algorithm are surprisingly ZeroR, J48, SimpleLogistics and SMO. 
